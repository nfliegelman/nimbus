...
**Post-audit priority order (audit batch 11, from the AUDIT_TODO section 22 table):** 1 nowcasting + midday cron, 2 NBM promotion decision at 50 ref settlements, 3 bet-confirmation logging, 4 EWMA + CRPS-sigma experiments at the 100-settlement retune, 5 Student-t tails at 500 tail buckets, 6 privacy/deploy: DECIDED batch 12 (public through paper, private = live gate 5; Pages-via-Actions one week post-deploy), 7 always-on runner pre-live. Explicit diminishing-returns list: more ensemble providers, extra surface variables, per-bucket CIs, portfolio optimization.
...
- [ ] **EWMA bias experiment** (audit batch 9): compare the decaying-average bias correction (Cui et al. style, weight ~0.05-0.10) against the rolling-30 window on held-out settlement MAE per city/kind; adopt only if it wins at 100+ settlements. Literature supports both; our data cannot yet prefer either.
...
- [ ] **Intraday nowcasting for same-day highs.** THE top value-of-information item (batch 9 literature verdict + community validation in REDDIT_FINDINGS.md Q3). Staged spec, prerequisites now shipped:
  - Station map: SHIPPED as `STATION_IDS` in code (batch 9), derived from the batch 1 CLI verification.
  - Mechanics: between ~9am and 2pm local, pull `api.weather.gov/stations/{id}/observations` since local 07:00 LST, compute the running observed max, then truncate every ensemble member for today's HIGH: member := max(member, running_max). Reprice buckets from the truncated cloud. Symmetric for LOW at a pre-dawn run if ever added.
  - Requires one new midday cron (~16:10 UTC, 11:10am CDT); do not bolt truncation onto existing runs, none sits in the binding window.
  - GATE before any play uses it: shadow-log truncated vs untruncated ladders for 30+ same-day HIGH events and require truncated CRPS/RPS to win; plays stay on untruncated pricing until then. This is the highest-value future feature and the closest thing to what professionals do. Between ~9am and 2pm local, pull live observations from the settlement station (NWS API: `api.weather.gov/stations/{ID}/observations`), truncate every ensemble member at the running max already observed, and re-price. The market is often slow to fold in the fact that, say, 91 has already printed. Requires mapping each city to its exact CLI station ID first.
- [ ] **HRRR + NBM blending decision.** Reference LOGGING shipped in audit batch 2: every record now stores NBM and HRRR CLI-day values (`ref`) so their skill vs the pooled ensemble is measurable from settlements. The open decision: once ~50 settlements with ref coverage exist, compare MAE of NBM / HRRR / ensemble mean per city and kind; if NBM wins, test anchoring the member cloud toward NBM (a weighted mean shift alongside the settlement-learned correction); if HRRR wins at lead 0-1, blend it for short leads only. Decide in audit batches 4-5 or at the 100-settlement retune, with numbers.
